...
nmea_s *
nmea_parse(char *sentence, size_t length, int check_checksum, char **values, int buf_size)
{
	...
	n_vals = _split_string_by_comma(val_string, values, /*ARRAY_LENGTH(values)*/ buf_size);
	if (0 == n_vals)
	{
		return (nmea_s *)NULL;
	}
