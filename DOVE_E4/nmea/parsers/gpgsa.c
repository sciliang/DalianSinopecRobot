...
#include "nmea/parsers/gpgsa.h"
#include "../parser_types.h"
#include "parse.h"

int nmea_gpgsa_init(nmea_parser_s *parser)
{
...
int nmea_gpgsa_allocate_data(nmea_parser_s *parser)
{
...
int nmea_gpgsa_set_default(nmea_parser_s *parser)
{
	memset(parser->data, 0, sizeof(nmea_gpgsa_s));
	return 0;
}

int nmea_gpgsa_free_data(nmea_s *data)
{
	free(data);
	return 0;
}

int nmea_gpgsa_parse(nmea_parser_s *parser, char *value, int val_index)
{
	...
	case NMEA_GPGSA_FIXTYPE:
		data->fixtype = (char)strtol(value, NULL, 10);
